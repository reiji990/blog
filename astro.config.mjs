...
export default defineConfig({
  ...
  // Astro 7 の既定は 'jsx'（React と同じ空白除去）。既存の出力 HTML を
  // 変えないため v5/v6 までの既定 true を明示する
  compressHTML: true,
  integrations: [mdx(), react()],
  vite: {
    plugins: [tailwindcss()],
    // Vite 既定の 'modules' (chrome87/edge88/es2020/firefox78/safari14) では
    // @astrojs/react の分割代入を変換できずビルドが落ちる。
    // package.json の browserslist (chrome>=93, safari>=15.4 等) に合わせる。
    // Vite 8 (Rolldown/oxc) では esbuild / optimizeDeps.esbuildOptions が
    // 非推奨になり、トランスフォーム対象も oxc 側が握るため build.target のみ残す
    ...
    // （Vite 8 で ssr.noExternal → resolve.noExternal に移動）
  ...
  markdown: {
    ...
    // Astro 7 の既定プロセッサは Sätteri。remark/rehype のプラグイン列を
    // そのまま使うため unified プロセッサを明示的に選ぶ
    processor: unified({
      gfm: false,
      smartypants: false,
      remarkPlugins: [
