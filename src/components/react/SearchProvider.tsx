...
// next/dynamic の代わりに React.lazy を使う。island 自体はクライアント専用なので
// ssr:false 相当の指定は不要。kbar 一式は検索インテントが発火するまで読まれない
...
export default function SearchProvider() {
  ...
  // Astro では children を島に渡さない（渡すとページ全体が React になる）。
  // この島はモーダルの器としてだけ存在し、ヘッダーの検索ボタンは
  // React を使わない .astro 側から search:open イベントを飛ばす
  return mounted ? (
